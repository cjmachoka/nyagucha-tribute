...
async function list(env) {
  const { results } = await env.DB.prepare(
    `SELECT id, caption, image_key, image_url, position, status, created_at
     FROM gallery_photos
...
async function create(request, env) {
  ...
  const r = await env.DB.prepare(
    `INSERT INTO gallery_photos (caption, image_key, image_url, position, status)
     VALUES (?, ?, ?, ?, 'published')`
  ).bind(caption, up.key, up.url, pos.n).run();
